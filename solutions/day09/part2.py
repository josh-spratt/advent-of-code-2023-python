...
def fill_up(matrix: List[List]) -> List[List]:
    ...
    for i in range(len(matrix) - 1, -1, -1):
        # Calculate the difference of the first element in the current list and the first element in the next list
        total = matrix[i - 1][0] - matrix[i][0]
        # Add the difference to the next list
        matrix[i - 1].insert(0, total)
    return matrix
...
def solve(file_path: str = "inputs/day09.txt") -> int:
    ...
    for line in lines:
        ...
        total_sum += matrix[0][0]
    return total_sum


if __name__ == "__main__":
    solution = solve()
    print(f"Day 9, Part 2: {solution}")
